...
(function () {
  ...
  roleButtons.forEach(btn => {
    btn.addEventListener('click', () => {
      roleButtons.forEach(r => r.classList.remove('active'));
      roleButtons.forEach(r => r.setAttribute('aria-selected','false'));
      btn.classList.add('active');
      btn.setAttribute('aria-selected','true');
      // TODO: load role-specific dashboard or content
      const role = btn.dataset.role;
      console.log('Switched role to', role);
      // keep feed the same but you may decide to load role-specific items
    });
  });

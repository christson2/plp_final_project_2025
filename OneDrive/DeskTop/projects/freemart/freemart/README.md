...
1. Start the database and backend (recommended):
	- Open this workspace in VS Code. A task labeled **Start Dev Servers** will run automatically on folder open and execute `scripts/start-dev.ps1` (PowerShell) to start Docker DB services and the backend dev server.
